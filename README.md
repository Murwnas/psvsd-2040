...
This is a module based on [Yinfanlu's PSVSD adapter](https://github.com/yifanlu/psvsd/). Last year, I took the original Eagle Autodesk files and adapted them to a [KiCad EDA source](https://github.com/Murwnas/psvsd-kicad).

One of the problems with the original PSVSD is the out-of-production Genesys chip. Apparently, from my research, all that chip does is expose a connected SD card to a USB Mass Storage device, which the Yinfanlu driver uses to interface with the PSVSD module. 
